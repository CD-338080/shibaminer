...
export default function Loading({ setIsInitialized, setCurrentView }: LoadingProps) {
  ...
  const fetchOrCreateUser = useCallback(async () => {
    try {
      ...
      const bypassTelegramAuth = process.env.NEXT_PUBLIC_BYPASS_TELEGRAM_AUTH === 'true';
      ...
      if (bypassTelegramAuth) {
        initData = initData || 'temp';
        telegramName = telegramName || 'Dev User';
        username = username || 'DevUser';
      }

      if (!initData) {
        throw new Error('initData is empty (open inside Telegram, or set NEXT_PUBLIC_BYPASS_TELEGRAM_AUTH=true)');
      }
      if (!telegramName) {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to fetch or create user (${response.status}): ${errorBody}`);
      }
  ...
  return (
    <div className="bg-[#1d2025] flex justify-center items-center h-screen">
      <div className="w-full max-w-xl text-white flex flex-col items-center">
        ...
        <h1 className="text-3xl font-bold mb-4">Loading Shiba Miner</h1>
